...
function FeaturedLocations() {
  ...
  return (
    <section id='trips' className="bg-white mx-auto me max-w-[115rem] px-4 sm:px-6 md:px-10 py-16 md:py-32">
      {/* Header */}
      <div className="mb-12 md:mb-20">
        <p className="text-lg md:text-xl mb-4 tracking-wide text-gray-800">03 FEATURED LOCATIONS</p>
          ...
          >
            <div className="relative overflow-hidden shadow-lg">
              <img
                src={location.image}
                alt={location.name}
                className="w-full h-64 sm:h-80 md:h-[650px] grayscale-0 hover:grayscale-0-0 transition object-cover duration-500 group-hover:scale-105"
              />
            </div>
            <div className="mt-4">
              <span className="text-lg md:text-2xl text-gray-500 font-mono">
                {(index + 1).toString().padStart(2, '0')}
              </span>
              <h3 className="text-xl sm:text-2xl md:text-3xl font-[500] text-black mt-2 group-hover:text-gray-700 transition duration-300">
                {location.name}
              </h3>
            </div>
          </Link>
